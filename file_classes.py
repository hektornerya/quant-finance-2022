# -*- coding: utf-8 -*-
"""
Created on Thu Apr 22 16:44:58 2021
...
class distribution_manager():
    ...
    def __init__(self, inputs):
        self.inputs = inputs # disribution_inputs
    ...
    def load_timeseries(self):
        ...
        if data_type == 'simulation':
            ...
            # directory = 'C:\\Users\\hp\\Desktop\\CESOREMIXQ\\HECTOR HERNANDEZ NERIA\\VBA\\DataScience\\Python\\Ejercicio\\FinanzasCuantitativasPy\\'
            ...
            ric = self.inputs.variable_name
            t = file_function.load_timeseries(ric)
...
class capm_manager():
    ...
    def plot_str(self):
        str_self = 'linnear regression / security ' + self.security\
    ...
    def load_timeseries(self):
        self.data_table = file_function.load_synchronised_timeseries(ric_x=self.benchmark, ric_y=self.security)
    ...
    def compute(self):
        ...
        self.null_hypothesis = p_value > 0.05 # p_value < 0.05 --> reject null hiypothesis
        self.correlation = np.round(r_value, self.nb_decimals) # correlation coefficient
        self.r_squared = np.round(r_value**2, self.nb_decimals) # pct of a variance of y explained by x
...
class hedge_manager():
    ...
    def __init__(self, inputs):
        ...
        self.security = inputs.security # Portfolo to hedge
        self.hedge_securities = inputs.hedge_securities # hedge universe
        self.nb_hedges = len(self.hedge_securities)
        self.delta_portfolio = inputs.delta_portfolio # in mn USD, default 10 
    ...
    def compute(self, regularisation=0.0):
        ...
        betas = self.betas
        optimal_result = minimize(fun=file_function.cost_function_hedge, x0=x, args=(self.delta_portfolio, self.beta_portfolio_usd, self.betas, regularisation))
        self.optimal_hedge = optimal_result.x
        # compute delta and beta of hedge portfolio
    ...
    def print_result(self, algo_type):
        print('------')
        print('Optimizacion result - ' + algo_type + ' solution')
        print('------')
...
class distribution_input():
    ...
    def __init__(self):
        ...
        self.variable_name= None # normal student VWS.CO exponentil chi-square
        self.degrees_freedom = None #only use in simulation student and chi square
...
class hedge_input:
    ...
    def __init__(self):
        ...
        self.security = 'BBVA.MC' # Portfolo to hedge
...
class portfolio_manager:
    ...
    def compute_covariance_matrix(self, bool_print=True):
        ...
        for i in range(size):
            ...
            for j in range(i+1):
                ric_y = rics[j]
                t = file_function.load_synchronised_timeseries(ric_x, ric_y)
        ...
        # COMPUTE EINGVALUES AND EINGVECTORS FOR SYMETRIC MATRIX
    ...
    def compute_portfolio(self, portfolio_type='default', target_return=None):
        ...
        if portfolio_type == 'min-variance':
            ...
            # initialize constarints
            cons = [{"type": "eq", "fun": lambda x: sum(abs(x)) - 1}] # unitary in norm L1
            # compute optimisation
            res= minimize(file_function.compute_portfolio_variance, x, args=(self.covariance_matrix), constraints=cons)
            ...
            # compute optimisation
            res= minimize(file_function.compute_portfolio_variance, x, args=(self.covariance_matrix), constraints=cons, bounds=bnds)
            weights_normalised = res.x
        ...
        portfolio.volatility_annual = file_function.compute_portfolio_volatility(weights_normalised, self.covariance_matrix)
        portfolio.volatility_annual_usd = file_function.compute_portfolio_volatility(weights, self.covariance_matrix)
...
class portfolio_item():
    ...
    def summary(self):
        ...
        if not self.pnl_annual_usd == None:
            print('Proffit and loss annual (mn USD): ' + str(self.pnl_annual_usd))
...
class montecarlo_item():
    ...
    def __str__(self):
        str_self = 'Monte Carlo simulation for option pricing | ' + self.call_or_put + '\n'\
            ...
            + 'price ' + str(self.mean) + '\n'\
            + 'probability of exercese ' + str(self.proba_exercise) + '\n'\
    ...
    def plot_histogram(self):
        ...
        dm.compute() # compute returns and all difernet risk metrics
        dm.plot_histogram() #plot histogrsm
        print(dm)
